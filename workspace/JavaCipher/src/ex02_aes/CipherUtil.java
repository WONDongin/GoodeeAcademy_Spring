...
public class CipherUtil {
	...
	private static byte[] makeKey(String key) {
		int len = key.length();
		char ch = 'A';
		for(int i = len; i < 16; i++) { // abc1234567 ABCDF
			key += ch++;
		}
	...
	public static void encryptFile(String plainFile, String cipherFile, String strkey) {
		try {
			...
			AlgorithmParameterSpec paramSpec = new IvParameterSpec(iv);
			cipher.init(Cipher.ENCRYPT_MODE, key, paramSpec); // 평문
			FileInputStream fis = new FileInputStream(plainFile); // 암호문
			...
			while((len = fis.read(buf)) != -1) { // 평문을 읽기
				cos.write(buf, 0, len); // 출력 스트림으로 암호화
			}
	...
	public static void decryptFile(String cipherFile, String plainFile, String strkey) {
		try {
			...
			ois.close();
			AlgorithmParameterSpec paramSpec = new IvParameterSpec(iv);
			// Cipher.DECRYPT_MOD : 복호화 모드
			cipher.init(Cipher.DECRYPT_MODE, key, paramSpec);
			// 암호화된 파일 읽기
			FileInputStream fis = new FileInputStream(cipherFile);
			// 복호화하여 plainFile 파일로 저장 plainText는 평문
